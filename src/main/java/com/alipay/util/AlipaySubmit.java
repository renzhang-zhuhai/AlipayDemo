...
public class AlipaySubmit {
	...
	/**
	 * 支付宝提供给商户的服务接入网关URL(新) The Alipay gateway provided to merchants
	 */
	...
	// The Alipay gateway of production environment.(pls use the below line instead
	// if you were in production environment)
	// private static final String ALIPAY_GATEWAY_NEW =
	// "https://mapi.alipay.com/gateway.do?";

	/**
	 * 生成签名结果 Generate the sign
	 * 
	...
	public static String buildRequestMysign(Map<String, String> sPara) {
		...
		// Rearrange parameters in the data set alphabetically and connect rearranged
		// parameters with & like "parametername=value"
	...
	/**
	 * 生成要请求给支付宝的参数数组 Generate a set of parameters need in the request of Alipay
	 * 
	...
	/**
	 * 建立请求，以表单HTML形式构造（默认） Build the request,costruct in the format of HTML form
	 * 
	 * @param sParaTemp     请求参数数组the request params
	 * @param strMethod     提交方式。两个值可选：post、get request form.support two types:post
	 *                      and get
	...
	/**
	 * 用于防钓鱼，调用接口query_timestamp来获取时间戳的处理函数 注意：远程解析XML出错，与服务器是否支持SSL等配置有关 Used to
	 * anti-phishing，use interface "query_timestamp" to get the function to get the
	 * timestamp note：If you get error when parsing XML from remote services,it is
	 * related to sever setting like whether the server supports SSL etc.
	 * 
